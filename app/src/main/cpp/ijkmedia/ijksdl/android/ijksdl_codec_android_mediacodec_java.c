...
static uint8_t *SDL_AMediaCodecJava_getOutputBuffer(SDL_AMediaCodec* acodec, size_t idx)
{
    ...
    SDL_AMediaCodec_Opaque *opaque = (SDL_AMediaCodec_Opaque *)acodec->opaque;
    output_buffer = J4AC_MediaCodec__getOutputBuffer__catchAll(env, opaque->android_media_codec, (jint)idx);
    if (J4A_ExceptionCheck__catchAll(env)) {
        ALOGE("%s: getOutputBuffer failed", __func__);
        return NULL;
    }

    uint8_t* bufferData = (uint8_t*)(*env)->GetDirectBufferAddress(env, output_buffer);
    if (J4A_ExceptionCheck__catchAll(env)) {
        ALOGE("%s: GetDirectBufferAddress failed", __func__);
        return NULL;
    }
    return bufferData;
}
